...
# Convert to a logical vector- filter to 1s or TRUE
nz_height_logical <- lengths(nz_height_sgbp) > 0
nz_height_logical

# Filter based on logical vector, same bracket notation
...
```{r}
# Left join and rename columns in select
...
```{r}
# Join, group by and summarize to get height levels 
...
```{r}
# Aggregating height levels by nz geometry
...
aggregating_zones_area-weighted <- st_interpolate_aw(x = incongruent_2,
                                                     aggregating_zones,
                                                     extensive = TRUE)
aggregating_zones_weighted$value
```
:::
...
# Alternative approach: aggregate(), all rows, total pop 15 column
regions2 <- aggregate(x = us_states[, "total_pop_15"], # data to be aggregated
                      by = list(us_states$REGION), # by region
                      FUN = sum, na.rm = TRUE) # sum pop within each region
...
# Combine geometries and western states
...
```{r}
# Union to combine buffer shapes
...
```{r}
# Middle point of nz polygon and seine line
nz_centroid <- st_centroid(nz)
seine_centroid <- st_centroid(seine)

# Middle point but ensure it is on the surface of geometry
...
```{r}
# Keep any point further than 2000 m from the line
