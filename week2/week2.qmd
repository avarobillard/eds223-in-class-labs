...
`sf` functions start with st_.

st_read(), st_point(), st_polygon(), st_drop_geometry(), st_crs(), st_transform(), etc.
...
```{r}
# Use st_transform() to change crs
...
```{r}
# Template code to check and change CRS
